...
QGCPopupDialog {
    ...
    readonly property real _dialogContentWidth: Math.min(root.maxContentAvailableWidth, Math.max(ScreenTools.defaultFontPixelWidth * 44, editRow.implicitWidth + (ScreenTools.defaultFontPixelWidth * 4)))
    ...
    QGCFlickable {
        ...
        height:         Math.min(root.maxContentAvailableHeight, editorContentColumn.implicitHeight)
        contentWidth:   width
        contentHeight:  editorContentColumn.implicitHeight
        clip:           true

        ColumnLayout {
            id:         editorContentColumn
            width:      parent.width
